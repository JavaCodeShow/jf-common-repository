 ...
 * string字符串 -> json
 * <p>
 * https://hub.fastgit.org/luoxn28/luo-leetcode/blob/master/src/main/java/com/luo/util/ToStringUtils.java
 */
public class StringToJsonUtils {
    ...
    public static Pattern datePattern = Pattern.compile(
            "^[a-zA-Z]{3} [a-zA-Z]{3} [0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} CST ((19|20)\\d{2})$");
    public static Pattern numPattern = Pattern.compile("^-?[0-9]+\\.?[0-9]*$");
    public static Pattern objectPattern = Pattern
            .compile("^[a-zA-Z0-9\\.]+\\(.+\\)$");
    public static Pattern listPattern = Pattern.compile("^\\[.*\\]$");
    public static Pattern mapPattern = Pattern.compile("^\\{.*\\}$");
    public static Pattern supperPattern = Pattern
            .compile("^super=[a-zA-Z0-9\\.]+\\(.+\\)$");
    ...
    public static String toJSONString(String toString) throws ParseException {
        Map<String, Object> stringObjectMap = toMap(toString);
     ...
     * string字符串 -> map
     */
    private static Map<String, Object> toMap(String toString)
        ...
        while (StringUtils.isNotEmpty(toString)
                && StringUtils.isNotEmpty(token = splitToken(toString))) {
            toString = StringUtils.removeStart(
                    StringUtils.removeStart(toString, token).trim(), ",")
                    .trim();
            ...
            map.put(keyValue.getKey(),
                    buildTypeValue(keyValue.getKey(), keyValue.getValue()));
        }
        return map;
    }
     ...
     */
    private static Object buildTypeValue(String key, String value)
            throws ParseException {
        ...
        if (datePattern.matcher(value).matches()) {
            return new SimpleDateFormat("EEE MMM dd HH:mm:ss Z yyyy",
                    new Locale("us")).parse(value).getTime();
        }
        // 数字类型
    ...
    private static Object buildListValue(String value) throws ParseException {
        ...
        while (StringUtils.isNotBlank(value)
                && StringUtils.isNotBlank(token = splitToken(value))) {
            result.add(buildTypeValue(null, token));
            value = StringUtils.removeStart(
                    StringUtils.removeStart(value, token).trim(), ",").trim();
        }
     ...
     * map类型
     */
    private static Map<Object, Object> buildMapValue(String value)
            throws ParseException {
        ...
        while (StringUtils.isNotEmpty(token = splitToken(value))) {
            Pair<String, String> keyValue = parseToken(token);
            result.put(buildTypeValue(keyValue.getKey(), keyValue.getKey()),
                    buildTypeValue(keyValue.getKey(), keyValue.getValue()));
    ...
    public static class ToStringTokenUtils {
        ...
        static Pair<String, String> parseToken(String token) {
            ...
            return new Pair<>(token.substring(0, index),
        ...
        private static int indexOfSplitToken(String token, char split) {
            ...
            for (int i = 0; i < token.length(); i++) {
                ...
                if (TOKEN_LEFT.contains(c)) {
                    ...
                    if (TOKEN_LEFT.indexOf(stack.peek()) != TOKEN_RIGHT
                            .indexOf(c)) {
                        throw new RuntimeException(
                                "splitFirstToken error, stack=" + stack
                                        + ", string字符串=" + token);
                    }
            ...
            throw new RuntimeException("splitFirstToken error, stack=" + stack
                    + ", string字符串=" + token);
        }
    }
}